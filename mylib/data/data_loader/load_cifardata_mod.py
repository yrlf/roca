...
def load_cifardata(dataset = "CIFAR10", num_workers = 0, batch_size = 128, add_noise = False, noise_type = None, flip_rate_fixed = None, random_state = 1, trainval_split=None, train_frac = 1, augment = True):
    ...
    # transform_train =    transforms.RandomCrop(random_crop, padding=4),transforms.RandomHorizontalFlip(), transforms.ToTensor(), normalize ])
    ...
    test_dataset = mylib.data.dataset.__dict__["CIFAR10_noise"](root=root, train=False, transform=transform_test, transform_eval=transform_test, add_noise = False, target_transform = None)
    train_val_dataset = mylib.data.dataset.__dict__["CIFAR10_noise"](
        root = root, 
        train = True, 
